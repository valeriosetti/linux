...
static irqreturn_t audin_fifo_isr(int irq, void *substream)
{
	...
	struct audin_fifo *fifo = snd_soc_dai_dma_data_get_capture(dai);
	unsigned int irq_val, new_int_val, start, end, ptr;
	...
	/* Update the IRQ triggering reference */
	start = snd_soc_component_read(dai->component, fifo->reg->start);
	end = snd_soc_component_read(dai->component, fifo->reg->end);
	ptr = snd_soc_component_read(dai->component, fifo->reg->ptr);
	new_int_val = round_down(ptr + AUDIN_FIFO_I2S_BLOCK, AUDIN_FIFO_I2S_BLOCK) - 1;
	if (new_int_val > end)
				...
				struct snd_soc_dai *dai)
{
	...
	ret = request_irq(fifo->audin_irq, audin_fifo_isr, 0,
				...
				struct snd_soc_dai *dai)
{
	...
	free_irq(fifo->audin_irq, substream);
	clk_disable_unprepare(fifo->pclk);
}
 ...
 */
static int audin_toddr_copy(struct snd_pcm_substream *substream,
			    ...
			    struct iov_iter *iter, unsigned long bytes)
{
	...
	if (bytes % 64) {
		dev_err(substream->pstr->dev,
			"Error: buffer size %lx not multiple of 64", bytes);
		return -EINVAL;
	}
	 ...
	 *    - the 24 bit case assumes that samples are still saved with 32
	 *      bits.
	 */
	...
	ret = copy_to_iter(ptr, bytes, iter);
	if (ret != bytes)
		return -EFAULT;
...
static int audin_toddr_dai_probe(struct snd_soc_dai *dai)
{
	...
	if (dai->id >= AUDIN_FIFO_COUNT) {
		dev_err(dai->dev, "Invalid DAI ID %d, maximum allowed is %d\n", 
			dai->id, AUDIN_FIFO_COUNT - 1);
		kfree(fifo);
		return -EINVAL;
	}
	...
	fifo->audin_irq = aiu->i2s.audin_irq;
