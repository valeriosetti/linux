...
/* This is the size of the FIFO (i.e. 64*64 bytes, as reported on the manual).
 * This value is connected to the interrupt's periodicity: an IRQ will be
 * generated when the input buffer has been written to multiple of this value.
 */
...
static irqreturn_t audin_fifo_isr(int irq, void *substream)
{
	...
	struct audin_fifo *fifo = snd_soc_dai_dma_data_get_capture(dai);
	unsigned int irq_val, old_irq_pos, new_irq_pos, start, end;
	...
	old_irq_pos = snd_soc_component_read(dai->component, fifo->reg->intr);
	new_irq_pos = old_irq_pos + AUDIN_FIFO_I2S_BLOCK;
	if (new_irq_pos > end)
		new_irq_pos = start + AUDIN_FIFO_I2S_BLOCK - 1;

	snd_soc_component_write(dai->component, fifo->reg->intr, new_irq_pos);
				...
				struct snd_soc_dai *dai)
{
	...
	if (ret) {
		...
		dev_err(dai->dev, "Failed to request IRQ %d\n", ret);
		return ret;
	}
