...
static int toddr_component_probe(struct snd_soc_component *component)
{
	struct toddr_comp_data *data = snd_soc_component_get_drvdata(component);

	return clk_bulk_prepare_enable(ARRAY_SIZE(clk_ids), data->clks);
}

static void toddr_component_remove(struct snd_soc_component *component)
{
	struct toddr_comp_data *data = snd_soc_component_get_drvdata(component);

	return clk_bulk_disable_unprepare(ARRAY_SIZE(clk_ids), data->clks);
}
...
static const struct snd_soc_component_driver toddr_component = {
	...
	.probe			= toddr_component_probe,
	.remove			= toddr_component_remove,
};

static int toddr_clk_get(struct device *dev, struct toddr_comp_data *data)
{
	int i, ret;
